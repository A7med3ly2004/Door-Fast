<div class="section-header"
                    ...
                    @forelse($deliveries as $d)
                        <tr id="delivery-row-{{ $d['id'] }}" class="delivery-row" data-phone="{{ $d['phone'] ?? '' }}"
                            ...
                            <td style="text-align: center;"><span class="badge"
                                    style="background:var(--blue-light);color:var(--blue)">أساسي</span></td>
                            ...
                            <td style="text-align: center;">
                                <div style="display:flex;gap:3px;justify-content: center;">
                                    <button class="btn btn-sm btn-info"
                                        onclick="openViewDelivery({{ json_encode($d) }})">عرض</button>
                ...
                <tbody>
                    @forelse($reserveDeliveries as $d)
                        <tr id="delivery-row-{{ $d['id'] }}" class="delivery-row" data-phone="{{ $d['phone'] ?? '' }}"
                            ...
                            <td style="text-align: center;">{{ $d['phone'] ?? '—' }}</td>
                            <td style="text-align: center;"><span class="badge"
                                    style="background:var(--red-light);color:var(--red)">احتياطي</span></td>
                            <td style="text-align: center;">
                                <button id="status-btn-{{ $d['id'] }}"
                                    onclick="toggleActive({{ $d['id'] }}, this, {{ json_encode($d) }})"
                                    style="display:inline-flex;align-items:center;gap:5px;padding:5px 12px;border-radius:20px;border:none;cursor:pointer;font-family:'Cairo',sans-serif;font-size:12px;font-weight:700;transition:all .2s ease;
            ...
            <div class="modal-body">
                ...
                <div style="margin-top:20px;text-align:center">
                    <button class="btn btn-warning"
                        style="width: 40%;text-align: center;background-color: #0f172a;border: 1px solid var(--border);color: white;justify-content: center;"
                        onclick="openIncentiveModal()">إعدادات شرائح الحوافز</button>
                </div>
            </div>
            <div class="modal-footer">
    ...
    {{-- View Modal --}}
    <div class="modal-overlay" id="modal-view-delivery">
        <div class="modal">
            <div class="modal-header" style="background:rgba(14, 165, 233, 0.08);border-bottom:0;">
                <h3 style="color:#0ea5e9;">عرض بيانات المندوب</h3><button class="btn-close"
                    onclick="closeModal('modal-view-delivery')">✕</button>
            </div>
            <div class="modal-body">
                <div class="form-row">
                    <div class="form-group"><label class="form-label">الاسم</label><input id="view-name" type="text"
                            class="form-control" readonly style="background: var(--bg-light); cursor: not-allowed;"></div>
                    <div class="form-group"><label class="form-label">كود المندوب</label><input id="view-code" type="text"
                            class="form-control" readonly style="background: var(--bg-light); cursor: not-allowed;"></div>
                    <div class="form-group"><label class="form-label">اسم المستخدم</label><input id="view-username"
                            type="text" class="form-control" readonly
                            style="background: var(--bg-light); cursor: not-allowed;"></div>
                    <div class="form-group"><label class="form-label">النوع</label><input id="view-role" type="text"
                            class="form-control" readonly style="background: var(--bg-light); cursor: not-allowed;"></div>
                </div>
                <div class="form-row">
                    <div class="form-group"><label class="form-label">الهاتف</label><input id="view-phone" type="text"
                            class="form-control" readonly style="background: var(--bg-light); cursor: not-allowed;"></div>
                ...
                <div class="form-group">
                    ...
                    <input id="view-active" type="text" class="form-control" readonly
                        style="background: var(--bg-light); cursor: not-allowed;">
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn btn-secondary" onclick="closeModal('modal-view-delivery')">إغلاق</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="modal-incentive-slices">
        <div class="modal modal-lg">
            ...
            <div class="modal-body">
                <p style="font-size:13px;color:var(--text-muted);margin-bottom:15px">تنبيه: ستبدأ الشريحة الأولى من رقم 1،
                    وكل شريحة تالية تتبع نهاية التي تسبقها.</p>
                <div class="table-wrap">
                    <table>
                        <thead>
                            <tr>
...
<script>
        ...
        window.openEditDelivery = function(d) {
        ...
        window.openViewDelivery = function(d) {
            document.getElementById('view-name').value = d.name;
            document.getElementById('view-code').value = d.code ?? '—';
            document.getElementById('view-username').value = d.username;
            document.getElementById('view-phone').value = d.phone ?? '—';
            document.getElementById('view-personal-phone').value = d.personal_phone ?? '—';
            document.getElementById('view-role').value = d.role === 'delivery' ? 'مندوب أساسي' : 'مندوب احتياطي';
            document.getElementById('view-active').value = d.is_active ? 'نشط' : 'غير نشط';
        ...
        window.renderSlicesForm = function() {
        ...
        window.toggleActive = async function(id, btn, d) {
        ...
        window.applyStatusBtn = function(btn, active) {
        ...
        window.applyShiftBtn = function(btn, active) {
            btn.dataset.active = active ? '1' : '0';
            if (active) {
                btn.style.background = 'rgba(34,197,94,.15)';
                btn.style.color = 'var(--success)';
        ...
        window.exportDeliveryExcel = function() {
            ...
            const columns = [
                ...
                { header: 'اجمالي التوصيل', key: 'revenue', width: 16 },
            ];
