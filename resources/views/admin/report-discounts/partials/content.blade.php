...
<div class="kpi-grid" style="margin-bottom:20px;grid-template-columns:repeat(2,1fr)">
    ...
    <div class="kpi-card red">
        <div class="kpi-label">🏷️ إجمالي الخصومات</div>
...
<div class="card" style="padding:0">
    <div style="padding:16px 20px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
        ...
        <span id="dc-count-label" style="font-size:13px;color:var(--text-muted)"></span>
    </div>
    <div class="table-wrap">
        <table>
            ...
            <tbody id="dc-orders-body">
                <tr><td colspan="10" style="text-align:center;padding:40px;color:var(--text-muted)">اضغط "عرض" لتحميل البيانات</td></tr>
            </tbody>
            ...
            if (!d.orders.length) {
                ...
                ordersBody.innerHTML = d.orders.map(function (o) {
                    return '<tr>' +
                        ...
                        '<td><span style="font-size:11px;color:var(--text-muted)">[' + o.client_code + ']</span> ' + escHtml(o.client) + '</td>' +
                        '<td>' + escHtml(o.callcenter) + '</td>' +
                        '<td>' + escHtml(o.delivery) + '</td>' +
                        '<td style="text-align:center"><span class="badge" style="background:var(--blue-light);color:var(--blue)">' + o.items_count + '</span></td>' +
                        '<td><strong style="color:var(--red)">' + parseFloat(o.discount).toFixed(2) + ' ج' +
                            (o.discount_type === 'percent' ? ' <small style="opacity:.7">(%)</small>' : '') +
                        '</strong></td>' +
                        '<td><strong>' + parseFloat(o.total).toFixed(2) + ' ج</strong></td>' +
                        '<td>' + statusBadge(o.status) + '</td>' +
                        '<td><button class="btn btn-sm btn-info" onclick="dcViewOrder(' + o.id + ')">👁 تفاصيل</button></td>' +
                    '</tr>';
                }).join('');
            }
        ...
        try {
            ...
            document.getElementById('dc-modal-body').innerHTML = `
                <div class="kpi-grid" style="grid-template-columns:repeat(3,1fr);margin-bottom:16px">
                    ...
                    <div class="kpi-card red">
                        <div class="kpi-label">🏷️ الخصم</div>
