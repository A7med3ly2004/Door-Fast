...
<script>
    ...
    async function loadReport(page = 1) {
        ...
        try {
            ...
            document.getElementById('delivery-breakdown').innerHTML = data.delivery_breakdown.length ? data.delivery_breakdown.map(d => `<tr><td style="text-align: right;">${d.name}</td><td style="text-align: center;">${d.total}</td><td style="text-align: center;"><span class="badge badge-green">${d.completed}</span></td><td style="text-align: center;"><div style="display:flex;align-items:center;gap:6px"><div style="flex:1;height:6px;background:var(--border);border-radius:3px"><div style="width:${d.rate}%;height:100%;background:var(--success);border-radius:3px"></div></div><span style="font-size:11px">${d.rate}%</span></div></td><td style="text-align: center;">${parseFloat(d.revenue).toFixed(2)} ج</td></tr>`).join('') : '<tr><td colspan="5" style="text-align:center;color:var(--text-muted)">لا بيانات</td></tr>';
            ...
            document.getElementById('report-orders').innerHTML = data.orders.length ? data.orders.map(o => `<tr><td style="color:var(--yellow); text-align: center;">${o.order_number}</td><td style="font-size:12px; text-align: center;">${formatDate(o.created_at)}</td><td style="text-align: center;">${o.client}</td><td style="text-align: center;">${o.callcenter}</td><td style="text-align: center;">${o.delivery}</td><td style="text-align: center;">${parseFloat(o.delivery_fee).toFixed(2)} ج</td><td style="text-align: center;">${parseFloat(o.discount).toFixed(2)} ج</td><td style="text-align: center;"><strong>${parseFloat(o.total).toFixed(2)} ج</strong></td><td style="text-align: center;">${statusBadge(o.status)}</td></tr>`).join('') : '<tr><td colspan="9" style="text-align:center;color:var(--text-muted)">لا طلبات</td></tr>';
            var t = data.totals;
