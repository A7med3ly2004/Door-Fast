    ...
    <div class="table-wrap">
        <table>
            ...
            <tbody id="clients-body">
                <tr>
                    <td colspan="8" style="text-align:center;padding:40px;color:var(--text-muted)">يرجى البحث عن عميل
                        (الاسم، الهاتف، أو الكود)</td>
                </tr>
            </tbody>
        </table>
    </div>
...
<div class="modal-overlay" id="modal-view">
    <div class="modal modal-lg">
        <div class="modal-header">
            <h3>👤 بيانات العميل</h3><button class="btn-close" onclick="closeModal('modal-view')">✕</button>
        </div>
    ...
    <div class="modal">
        ...
        <form id="form-add-client" onsubmit="saveClient(event)">
            <div class="modal-body" style="padding:24px; background:var(--bg); border-radius:0 0 12px 12px;">
                <div
                    style="background:rgba(255,255,255,0.02); border:1px solid var(--border); border-radius:12px; padding:20px; margin-bottom:20px;">
                    <div
                        style="font-size:14px; font-weight:700; color:var(--text-muted); margin-bottom:16px; display:flex; align-items:center; gap:8px;">
                    ...
                    <div class="form-group">
                        <label class="form-label" style="font-size:13px;">الاسم الكامل <span
                                style="color:var(--red)">*</span></label>
                        <input type="text" name="name" class="form-control" required placeholder="اسم العميل"
                            style="background:var(--bg); border:1px solid var(--border); padding:10px 14px; border-radius:8px;">
                    </div>
                    <div class="form-row" style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                        <div class="form-group">
                            <label class="form-label" style="font-size:13px;">رقم الهاتف <span
                                    style="color:var(--red)">*</span></label>
                            <input type="text" name="phone" id="new-phone" class="form-control" required
                                placeholder="رقم الموبايل"
                                style="background:var(--bg); border:1px solid var(--border); padding:10px 14px; border-radius:8px; direction:ltr; text-align:right;">
                        </div>
                        <div class="form-group">
                ...
                <div
                    style="background:rgba(255,255,255,0.02); border:1px solid var(--border); border-radius:12px; padding:20px;">
                    <div
                        style="font-size:14px; font-weight:700; color:var(--text-muted); margin-bottom:16px; display:flex; align-items:center; gap:8px;">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        <label class="form-label" style="font-size:13px;">العنوان بالتفصيل <span
                                style="color:var(--red)">*</span></label>
                        <input type="text" name="first_address" class="form-control" required
                            placeholder="مثال: مجاورة 12 - امام بنك التعمير و الاسكان"
                            style="background:var(--bg); border:1px solid var(--border); padding:10px 14px; border-radius:8px;">
                    </div>
                </div>
            </div>
            <div class="modal-footer"
                style="padding:16px 24px; border-top:1px solid var(--border); background:var(--bg); border-radius:0 0 12px 12px; gap:12px; display:flex; justify-content:flex-end;">
                <button type="button" class="btn btn-secondary" onclick="closeModal('modal-add-client')"
                    style="padding:10px 24px; border-radius:8px;">إلغاء</button>
                <button type="submit" class="btn btn-success" id="btn-save-client"
                    style="padding:10px 24px; border-radius:8px; font-weight:600;">حفظ البيانات</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-overlay" id="modal-edit-client">
    <div class="modal">
        ...
        <form id="form-edit-client" onsubmit="updateClient(event)">
            ...
            <div class="modal-body" style="padding:24px; background:var(--bg); border-radius:0 0 12px 12px;">
                ...
                <div
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
                        <div
                            style="font-size:14px; font-weight:700; color:var(--text-muted); display:flex; align-items:center; gap:8px;">
                            <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z">
                                </path>
                        ...
                        <button type="button" class="btn btn-sm btn-secondary" onclick="addAddressRow()"
                            style="display:flex; align-items:center; gap:4px; border-radius:6px; padding:6px 10px;">
            ...
            <div class="modal-footer"
                style="padding:16px 24px; border-top:1px solid var(--border); background:var(--bg); border-radius:0 0 12px 12px; gap:12px; display:flex; justify-content:flex-end;">
                <button type="button" class="btn btn-secondary" onclick="closeModal('modal-edit-client')"
                    style="padding:10px 24px; border-radius:8px;">إلغاء</button>
                <button type="submit" class="btn btn-primary" id="btn-update-client"
                    style="padding:10px 24px; border-radius:8px; font-weight:600;">حفظ التعديلات</button>
            </div>
        </form>
    </div>
</div>
