...
<script>
    ...
    function saveDrafts() {
        ...
        document.querySelectorAll('.order-card').forEach(card => {
            ...
            drafts.push({
                ...
                addrSelHtml: document.getElementById(id + '-address-sel')?.innerHTML || '', addrSelVal: document.getElementById(id + '-address-sel')?.value || '', addrSelDisplay: document.getElementById(id + '-address-sel')?.style.display || '', addrTxt: document.getElementById(id + '-address-txt')?.value || '', addrTxtDisplay: document.getElementById(id + '-address-txt')?.style.display || '', isNewAddr: document.getElementById(id + '-is-new-addr')?.value || '0', clientDeliveryLink: document.getElementById(id + '-client-delivery-link')?.value || '',
                delivery: document.getElementById(id + '-delivery')?.value || '', stOpen: document.getElementById(id + '-sendto')?.classList.contains('open') || false, stPhone: document.getElementById(id + '-st-phone')?.value || '', stPhone2: document.getElementById(id + '-st-phone2')?.value || '', stAddrWrap: document.getElementById(id + '-st-addr-wrap')?.innerHTML || '', stAddrSelVal: document.getElementById(id + '-st-addr-sel')?.value || '', stAddrTxtDisplay: document.getElementById(id + '-st-addr-txt')?.style.display || '', stAddrVal: document.getElementById(id + '-st-addr-txt')?.value || '', stCode: document.getElementById(id + '-st-code')?.value || '', stName: document.getElementById(id + '-st-name')?.value || '', stClientId: document.getElementById(id + '-st-client-id')?.value || '', stClientFound: document.getElementById(id + '-st-client-found')?.value || '0', stDeliveryLink: document.getElementById(id + '-st-delivery-link')?.value || '',
                notes: document.getElementById(id + '-notes')?.value || '', fee: document.getElementById(id + '-fee')?.value || '', disc: document.getElementById(id + '-disc')?.value || '0', discType: document.getElementById(id + '-disc-type')?.value || 'amount', items: items, openedAt: document.getElementById(id + '-opened-at')?.value || '', displayTime: document.getElementById(id + '-display-time')?.innerText || ''
            });
        });
    ...
    function nowStr() { return new Date().toLocaleString('en-GB', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: true }).replace('am', 'ص').replace('pm', 'م').replace('AM', 'ص').replace('PM', 'م'); }
    ...
    <div class="order-card-footer">
        ...
        if (!draft) { for (let i = 0; i < 1; i++) addItemRow(id); resetAddressSection(id, false); saveDrafts(); }
        else {
    ...
    function onStAddressChange(cardId) {
        var sel = document.getElementById(cardId + '-st-addr-sel'); var txt = document.getElementById(cardId + '-st-addr-txt');
        if (sel && sel.value === '__new__') { txt.style.display = ''; txt.focus(); txt.value = ''; } else if (txt) { txt.style.display = 'none'; } saveDrafts();
    }
    ...
    async function clearCard(cardId) {
        ...
        ['phone', 'phone2', 'code', 'name', 'st-phone', 'st-phone2', 'st-name', 'st-code', 'notes', 'fee'].forEach(f => { if (document.getElementById(cardId + '-' + f)) document.getElementById(cardId + '-' + f).value = ''; }); document.getElementById(cardId + '-client-id').value = ''; document.getElementById(cardId + '-client-found').value = '0'; document.getElementById(cardId + '-disc').value = '0'; document.getElementById(cardId + '-items').innerHTML = ''; resetAddressSection(cardId, false); calcTotals(cardId); for (let i = 0; i < 1; i++) addItemRow(cardId); saveDrafts();
    }
    ...
    async function saveCard(cardId) {
        ...
        var clientDeliveryLink = document.getElementById(cardId + '-client-delivery-link')?.value.trim() || ''; var deliveryId = document.getElementById(cardId + '-delivery').value; var stOpen = document.getElementById(cardId + '-sendto')?.classList.contains('open'); var sendToDeliveryLink = ''; var sendToPhone = ''; var sendToPhone2 = ''; var sendToAddr = ''; var sendToCode = ''; var sendToName = ''; var sendToClientId = ''; if (stOpen) { sendToPhone = document.getElementById(cardId + '-st-phone')?.value.trim() || ''; sendToPhone2 = document.getElementById(cardId + '-st-phone2')?.value.trim() || ''; var stSel = document.getElementById(cardId + '-st-addr-sel'); var stTxt = document.getElementById(cardId + '-st-addr-txt'); if (stSel && stSel.style.display !== 'none') { sendToAddr = stSel.value === '__new__' ? (stTxt?.value.trim() || '') : stSel.value; } else if (stTxt) { sendToAddr = stTxt.value.trim(); } sendToCode = document.getElementById(cardId + '-st-code')?.value.trim() || ''; var rawName = document.getElementById(cardId + '-st-name')?.value.trim(); sendToName = rawName ? rawName : 'Unnamed'; sendToClientId = (document.getElementById(cardId + '-st-client-found')?.value === '1') ? document.getElementById(cardId + '-st-client-id')?.value : ''; sendToDeliveryLink = document.getElementById(cardId + '-st-delivery-link')?.value.trim() || ''; }
        ...
        document.getElementById(cardId + '-items').querySelectorAll('tr').forEach(tr => {
            const itemName = tr.cells[0].querySelector('input')?.value.trim();
            const qty = parseFloat(tr.cells[1].querySelector('input')?.value) || 0;
            const price = parseFloat(tr.cells[2].querySelector('input')?.value) || 0;
            const shopSel = tr.querySelector('select[id$="-shop"]');
            const shopId = shopSel ? shopSel.value : null;
            if (itemName) {
                if (!shopId) missingShop = true;
                items.push({ item_name: itemName, quantity: qty, unit_price: price, shop_id: shopId || null });
            }
        });
        if (!phone) { showError('رقم الهاتف مطلوب'); return; } if (!code) { showError('الكود مطلوب'); return; } if (!name) { showError('اسم العميل مطلوب'); return; } if (!clientAddress) { showError('العنوان مطلوب'); return; }
        // ✅ التحقق من عنوان المستلم عند فتح قسم "إرسال إلى عميل آخر"
        if (stOpen && !sendToAddr) { showError('عنوان العميل المستلم مطلوب'); return; }
        if (!items.length) { showError('يجب إضافة صنف واحد على الأقل'); return; }
        if (missingShop) { showError('يجب اختيار المتجر لكل صنف'); return; }
        var feeInput = document.getElementById(cardId + '-fee').value; if (feeInput === '') { showError('رسوم التوصيل مطلوبة'); return; }
        ...
        var payload = { phone, phone2, code, name, client_address: clientAddress, is_new_address: isNewAddr, delivery_id: deliveryId || null, send_to_phone: sendToPhone || null, send_to_phone2: sendToPhone2 || null, send_to_address: sendToAddr || null, send_to_code: sendToCode || null, send_to_name: sendToName || null, send_to_client_id: sendToClientId || null, client_delivery_link: clientDeliveryLink || null, send_to_delivery_link: sendToDeliveryLink || null, notes, delivery_fee: fee, discount: disc, discount_type: discType, items, opened_at: openedAt };
        try {
