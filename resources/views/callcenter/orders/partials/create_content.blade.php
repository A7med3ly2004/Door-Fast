...
<script>
    ...
    function saveDrafts() {
        ...
        document.querySelectorAll('.order-card').forEach(card => {
            ...
            drafts.push({
                ...
                delivery: document.getElementById(id + '-delivery')?.value || '', stOpen: document.getElementById(id + '-sendto')?.classList.contains('open') || false, stPhone: document.getElementById(id + '-st-phone')?.value || '', stAddrWrap: document.getElementById(id + '-st-addr-wrap')?.innerHTML || '', stAddrVal: document.getElementById(id + '-st-addr-txt')?.value || '',
    ...
    async function searchSendTo(cardId) {
    ...
    async function saveCard(cardId) {
        ...
        var deliveryId = document.getElementById(cardId + '-delivery').value; const sendToPhone = document.getElementById(cardId + '-st-phone')?.value.trim() || ''; let sendToAddr = ''; const stEl = document.getElementById(cardId + '-st-addr-txt'); if (stEl) sendToAddr = stEl.value || stEl.options?.[stEl.selectedIndex]?.value || '';
        ...
        var payload = { phone, phone2, code, name, client_address: clientAddress, is_new_address: isNewAddr, delivery_id: deliveryId || null, send_to_phone: sendToPhone || null, send_to_address: sendToAddr || null, notes, delivery_fee: fee, discount: disc, discount_type: discType, items };
        try {
