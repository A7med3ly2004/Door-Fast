...
function renderReceivedOrders() {
    ...
    cachedOrders.forEach(order => {
        ...
        card.innerHTML = `
            ...
            <button class="btn-view" onclick="openOrderModal(${order.id})">📋 عرض الطلب بالكامل</button>
        `;
...
function openOrderModal(orderId) {
    ...
    var deliveryAddressHtml = order.client_address || 'لم يتم تحديده';
    if (order.send_to_phone) {
        deliveryAddressHtml = `
    ...
    modalContent.innerHTML = `
        ...
        <div class="modal-body">
            <div class="info-group"><div class="info-label">بيانات العميل المالك</div><div class="info-value">${clientName}</div><div class="info-value" style="margin-top:5px">${phoneHtml}</div></div>
