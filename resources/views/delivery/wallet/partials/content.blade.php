{{-- Delivery wallet statement partial --}}
<style>
    .w-filter-bar {
        display: flex;
        gap: 10px;
...
<div class="w-card" style="margin-bottom:16px;">
    <div class="w-filter-bar">
        <div>
            ...
            <input type="date" id="dw-from" class="w-input">
        </div>
        <div>
            <span class="w-label">إلى تاريخ</span>
            <input type="date" id="dw-to" class="w-input">
        </div>
        <button class="w-btn w-btn-primary" onclick="dwApply()">عرض</button>
        <button class="w-btn w-btn-secondary" onclick="dwReset()">تصفير</button>
    </div>
</div>
...
<div class="w-kpi-grid">
    <div class="w-kpi green">
        <div class="w-kpi-label">المدين</div>
        <div class="w-kpi-value" id="dw-kpi-debit" style="color:var(--success);">—</div>
        <div class="w-kpi-sub">ج.م</div>
    </div>
    <div class="w-kpi red">
        <div class="w-kpi-label">الدائن</div>
        <div class="w-kpi-value" id="dw-kpi-credit" style="color:var(--secondary);">—</div>
        <div class="w-kpi-sub">ج.م</div>
    </div>
    <div class="w-kpi">
        <div class="w-kpi-label">الرصيد الحالي</div>
        <div class="w-kpi-value" id="dw-kpi-balance" style="color:var(--primary);">—</div>
        <div class="w-kpi-sub">ج.م</div>
    </div>
</div>
...
<div class="w-card" style="padding:0;">
    <div
        style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid var(--border-color);">
        <span style="font-size:14px;font-weight:700;">سجل العمليات</span>
        <span class="badge" id="dw-badge" style="background:#e5e7eb;color:#475569;">—</span>
    </div>
    <div class="w-table-wrap">
        <table class="w-table">
            ...
            <tbody id="dw-tbody">
                <tr>
                    <td colspan="6" style="text-align:center;color:var(--text-muted);padding:40px;">جاري التحميل...</td>
                </tr>
            </tbody>
        </table>
    </div>
...
<script>
    (function () {
        ...
        const BASE = '/delivery/wallet/statement';

        async function fetchStatement() {
            const tbody = document.getElementById('dw-tbody');
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--text-muted);padding:40px;">جاري التحميل...</td></tr>';
            try {
                const params = {};
                const f = document.getElementById('dw-from').value;
                const t = document.getElementById('dw-to').value;
                if (f) params.from = f;
                if (t) params.to = t;
                ...
                document.getElementById('dw-kpi-debit').textContent = summary.total_debit;
                document.getElementById('dw-kpi-credit').textContent = summary.total_credit;
                document.getElementById('dw-kpi-balance').textContent = summary.current_balance;
                document.getElementById('dw-badge').textContent = transactions.length + ' عملية';
        ...
        window.dwApply = function () { fetchStatement(); };
        window.dwReset = function () {
            document.getElementById('dw-from').value = '';
            document.getElementById('dw-to').value = '';
            fetchStatement();
        };
