...
<div class="empty-state" id="new-reserve-empty-state" style="display:none">
    ...
    <p>سيتم إعلامك تلقائياً عند وجود طلب جديد لم يُقبل</p>
</div>
...
<script>
    ...
    function fetchNewOrders() {
        ...
        axios.get('{{ route("reserve.orders.new-data") }}').then(res => { currentOrders = res.data.orders; renderNewOrders(); });
    }
    function renderNewOrders() {
        ...
        currentOrders.forEach(order => {
            ...
            card.innerHTML = `<div class="order-header"><div class="order-number">#${order.order_number}</div>${alertHtml}</div><div class="hidden-zone"><div class="blur-overlay"><span style="font-size:18px">🔒</span></div><div class="hidden-content"><strong>الاسم:</strong> ${order.client?.name ?? ''}<br><strong>الهاتف:</strong> ${order.client?.phone ?? ''}</div></div><button class="btn-accept" onclick="acceptOrder(${order.id})">✔ قبول الطلب</button>`;
    ...
    function acceptOrder(id) {
        Swal.fire({ title: 'تأكيد', text: 'هل أنت متأكد من قبول الطلب؟', icon: 'question', showCancelButton: true, confirmButtonText: 'نعم', cancelButtonText: 'إلغاء' }).then(result => {
            ...
            axios.post(`/reserve/orders/${id}/accept`).then(res => {
                if (res.data.success) { Swal.fire({ icon: 'success', title: 'تم القبول', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 }); currentOrders = currentOrders.filter(o => o.id !== id); renderNewOrders(); checkNewOrdersBadge(); }
